...
public class TwitchClient : MonoBehaviour
{
    ...
    private void OnWhisperCommandReceived(object sender, TwitchLib.Client.Events.OnWhisperCommandReceivedArgs e)
    {
        ...
        {
            case "hello":
                _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"Whale hello there, {e.Command.WhisperMessage.DisplayName}!");
                break;

            case "about":
                _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"Running Meatball Mayhem v0.9a! Whisper !commands to view the game commands or download the app!");
                break;

            default:
                _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"Unknown chat command: {e.Command.CommandIdentifier}{e.Command.CommandText}");
                break;

            case "commands":
                _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"Commands: !hello, !about, !join, !moveleft, !moveright, !jumpleft, !jumpright, !aim, !fire");
                break;
            ...
            case "join":
                ...
                {
                    _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"You've already joined and can't join again!");
                    break;
                }

                if (pm.atMaxPlayers)
                {
                    _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"Game is now full! Try joining after the game is over!");
                    Debug.Log("A chatter tried to join the game but game is full...");
                    break;
                }
                _client.SendWhisper(e.Command.WhisperMessage.DisplayName, $"Joining the game!");
                AddNewPlayer(name);
                ...
                // Check to see if the game is joinable
                // We do this again after a player joins to see if they're taking the last open player spot
                StartCoroutine(pm.CheckForMaxPlayers(pm.atMaxPlayers));
            ...
            case "aim":
                ...
                if (GameObject.Find(name))
                {
                    ...
                    if (rm.phase == "Attack" && isPlayerAlive)
                    {
                        ...
                        // Do a check to see if the angle is within physics boundaries.
            ...
            case "fire":
                ...
                if (GameObject.Find(name))
                {
                    ...
                    if (rm.phase == "Attack" && isPlayerAlive)
                    {
                        ...
                        {
                            // Do a check to see if the value is within reasonable bounds.
                            if (forceValueInt > 0 && forceValueInt <= 5)
                            {
